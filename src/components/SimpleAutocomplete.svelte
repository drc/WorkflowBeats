<script>
    ...
    $effect(() => {
        if (selectedItem) untrack(() => onSelectedItemChanged());
    });
    ...
    let hasSelection = $derived((multiple && selectedItem?.length > 0) || (!multiple && selectedItem));
        ...
        {#if filteredListItems && filteredListItems.length > 0}
            ...
            {#each filteredListItems as listItem, i}
                {#if listItem && (maxItemsToShowInList <= 0 || i < maxItemsToShowInList)}
                    ...
                    >
                        {@render item?.(listItem.item, listItem.highlighted ? listItem.highlighted : listItem.label)}
                        {#if !item}
